...
<div class="module-header">
    <div class="header-info">
        <h3>Add Product</h3>
        <p>Create a new product with SKU and barcode (auto-generated if left empty).</p>
    </div>
...
<style>
    ...
    .modal { width:100%; max-width:560px; background:#fff; border-radius:14px; overflow:hidden; border:1px solid rgba(17,24,39,0.1); box-shadow:0 20px 50px rgba(0,0,0,0.2); }
    .modal-header { padding:16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #e5e7eb; background:#f9fafb; }
    ...
    <form method="POST" action="{{ route('admin.inventory.products.store') }}" enctype="multipart/form-data">
        ...
        <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:14px;">
            <div style="grid-column: span 2;">
                ...
                <input name="name" value="{{ old('name') }}" required style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
            </div>

            <div>
                <label style="display:block; font-weight:900; margin-bottom:6px;">SKU *</label>
                <input name="sku" value="{{ old('sku') }}" required style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
            </div>
            <div>
                <label style="display:block; font-weight:900; margin-bottom:6px;">Barcode (optional)</label>
                <input name="barcode" value="{{ old('barcode') }}" style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
            ...
            <div>
                ...
                <input name="category" value="{{ old('category') }}" style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
            </div>
            <div>
                <label style="display:block; font-weight:900; margin-bottom:6px;">Image URL</label>
                <input name="image_url" value="{{ old('image_url') }}" placeholder="https://..." style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
            ...
            <div style="grid-column: span 2;">
                ...
                <textarea name="description" rows="3" style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">{{ old('description') }}</textarea>
            </div>

            <div>
                <label style="display:block; font-weight:900; margin-bottom:6px;">Cost Price *</label>
                <input type="number" step="0.01" min="0" name="cost_price" value="{{ old('cost_price', '0') }}" required style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
            </div>
            <div>
                <label style="display:block; font-weight:900; margin-bottom:6px;">Selling Price *</label>
                <input type="number" step="0.01" min="0" name="selling_price" value="{{ old('selling_price', '0') }}" required style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
            </div>

            <div>
                <label style="display:block; font-weight:900; margin-bottom:6px;">Qty On Hand *</label>
                <input type="number" step="0.01" min="0" name="qty_on_hand" value="{{ old('qty_on_hand', '0') }}" required style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
            </div>
