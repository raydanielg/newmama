...
<div class="module-header">
    <div class="header-info">
        <h3>{{ $title }}</h3>
        <p>Open customer invoices grouped by aging bucket.</p>
    </div>
...
<div class="content-card" style="padding:16px; margin-bottom:14px;">
    <form method="GET" action="{{ route('admin.reports.ar-aging') }}" style="display:flex; gap:10px; flex-wrap:wrap;">
        <input type="date" name="as_of" value="{{ $asOf }}" style="padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
        <button class="btn-primary" type="submit">Run</button>
        <a class="btn-icon" style="text-decoration:none;" href="{{ route('admin.reports.ar-aging') }}">Reset</a>
    </form>
</div>
    ...
    <div class="table-responsive">
        <table class="admin-table">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="6" style="text-align:center; color:#6b7280; padding:18px;">No open AR found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
