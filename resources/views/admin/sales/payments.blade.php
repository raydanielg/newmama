...
<div class="module-header">
    <div class="header-info">
        <h3>{{ $title }}</h3>
        <p>Customer receipts and payment reporting.</p>
    </div>
    <div class="header-actions" style="display:flex; gap:10px;">
        <a href="{{ route('admin.vouchers.cash-receipt.create') }}" class="btn-primary" style="text-decoration:none;">New Receipt</a>
        <a href="{{ route('admin.sales.payments.csv', request()->query()) }}" class="btn-icon" style="text-decoration:none;">Export CSV</a>
    </div>
</div>

<div class="content-card" style="padding: 16px;">
    <form method="GET" action="{{ route('admin.sales.payments') }}" style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:14px;">
        ...
        <a class="btn-icon" style="text-decoration:none;" href="{{ route('admin.sales.payments') }}">Reset</a>
    </form>
                ...
                @foreach($vouchers as $v)
                    <tr>
                        ...
                        <td style="text-align:right; display:flex; justify-content:flex-end; gap:8px;">
                            @if($v->customer_id)
                                <a class="btn-icon" style="text-decoration:none;" href="{{ route('admin.customers.ledger', $v->customer_id) }}">Profile</a>
                            @endif
                            <a class="btn-icon" style="text-decoration:none;" href="{{ route('admin.vouchers.view', $v) }}">Receipt</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
