...
<div class="module-header">
    <div class="header-info">
        <h3>{{ $title }}</h3>
        <p>Create draft debit notes for customers (posting workflow can be added later).</p>
    </div>
    <div class="header-actions">
        <a class="btn-icon" style="text-decoration:none;" href="{{ route('admin.vouchers.credit-note.create') }}">New Credit Note</a>
    </div>
</div>

<div class="content-card" style="padding:16px; margin-bottom:14px;">
    <form method="POST" action="{{ route('admin.sales.debit-note.store') }}" style="display:grid; grid-template-columns: 160px 1fr 220px 180px 140px; gap:10px; align-items:end;">
        @csrf
        <div>
        ...
        <div>
            ...
            <input name="description" value="{{ old('description') }}" required placeholder="Debit note details" style="width:100%; padding:10px 12px; border:1px solid #e5e7eb; border-radius:10px;">
        </div>
        <div>
    ...
    <div class="table-responsive">
        <table class="admin-table">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="7" style="text-align:center; color:#6b7280; padding:18px;">No debit notes yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
